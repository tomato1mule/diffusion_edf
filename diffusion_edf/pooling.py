...
class RadiusGraph(torch.nn.Module):
    ...
    def forward(self, node_coord_src: torch.Tensor, node_feature_src: torch.Tensor, node_attr_src: torch.Tensor, batch_src: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        assert node_coord_src.ndim == 2 and node_coord_src.shape[-1] == 3

        node_coord_dst = node_coord_src
        batch_dst = batch_src
        node_attr_dst = node_attr_src
        ...
        return node_coord_dst, node_feature_dst, node_attr_dst, edge_src, edge_dst, degree, batch_dst
...
class FpsPool(torch.nn.Module):
    ...
    def forward(self, node_coord_src: torch.Tensor, node_feature_src: torch.Tensor, node_attr_src: torch.Tensor, batch_src: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        node_attr_dst = node_attr_src.index_select(index=node_dst_idx, dim=0)
